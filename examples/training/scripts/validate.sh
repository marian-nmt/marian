...
cat $dev | ../../build/amun -c $prefix.dev.npz.amun.yml -b 12 -n --mini-batch 10 --maxi-batch 100 2>/dev/null \
 | sed 's/\@\@ //g' | ./moses-scripts/scripts/recaser/detruecase.perl > output.postprocessed

## get BLEU
./moses-scripts/scripts/generic/multi-bleu.perl $ref < output.postprocessed | cut -f 3 -d ' ' | cut -f 1 -d ','
