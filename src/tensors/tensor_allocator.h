...
private:
  ...
  void reset(Tensor t, float* start) { t->reset(start); }
  ...
  void resetAllocated(float* oldStart) {
    ...
    if(allocated_.empty()) {
      ...
      size_t used = allocated_.back()->data() - device_.data()
                    + allocated_.back()->size();
...
public:
  TensorAllocator(size_t device) : device_(device) {
    lastGap_ = {device_.capacity(), device_.data()};
    gaps_.insert(lastGap_);
  }
  ...
  size_t capacity() { return device_.capacity(); }
