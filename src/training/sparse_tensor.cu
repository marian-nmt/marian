...
__global__ void gFindSubtensor(int* indices,
  ...
  SparseTensorBase::SparseTensorBase(float* data, int* indices, int size, size_t device) {
    data_ = data;
    indices_ = indices;
    size_ = size;
    capacity_ = size;
  ...
  int SparseTensorBase::capacity() { return capacity_; }

  int SparseTensorBase::size() { return size_; }

  float* SparseTensorBase::data() { return data_; }

  int* SparseTensorBase::indices() { return indices_; }

  void SparseTensorBase::copyFrom(float* data, int* indices, int size, bool data_only) {
    if(capacity_ < size) {
      return;
      // NO enough capacity
    }
  ...
  void SparseTensorBase::copyFrom(std::shared_ptr<SparseTensorBase> t, bool data_only) {
    copyFrom(t->data(), t->indices(), t->size(), data_only);
  }

  size_t SparseTensorBase::getDevice() { return device_; }

  void SparseTensorBase::setSize(int size) { size_ = size; }
  ...
  std::shared_ptr<SparseTensorBase> SparseTensorBase::subtensor(int pos, int size, int idx) {
    cudaSetDevice(device_);
    cudaStreamSynchronize(0);
