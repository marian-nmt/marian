...
class GradientDropBase {
  ...
  void grad_drop_do(
    ...
    thrust::sort(dev_data_ptr,
                 dev_data_ptr + sortSize);  // OVERKILL. Too slow and need extra
                                            // memory. Need to replace with
                                            // faster k-th selection. (inplace
                                            // Radix Select?)
...
public:
  void dropGraph(Tensor t, SparseTensor destination, double rate = 0.99) {
    ...
    if (step < 10){
      ...
      std::cerr<<"GPU :"<< t->getDevice()<<"  |  overall dropping "<<(float)x /
    t->size() <<std::endl;
    }*/
    step++;
  }
