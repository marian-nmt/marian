...
__HDI__ C<N == K> grad(Var<N>, Var<K>) {
  return C<N == K>();
}
...
template <int N>
__HDI__ C<0> grad(Capture, Var<N>) {
  return C<0>();
}
...
template <typename X, typename Y, int N>
__HDI__ auto grad(Mult<X, Y> f, Var<N> g)->decltype((grad(f.x, g) * f.y + f.x * grad(f.y, g))) {
...
template <typename X, int N>
__HDI__ auto grad(Exp<X> f, Var<N> g)->decltype((f * grad(f.x, g))) {
  return (f * grad(f.x, g));
}
...
int main(int argc, char** argv) {
  ...
  auto f = exp(x) / (C<1>() + exp(x));
  auto df = grad(f, y);


  std::cerr << f(1.f) << " " << sizeof(f) << " " << sizeof(simple(f)) << " " << sizeof(simple(simple(f))) << std::endl;
  std::cerr << df(1.f) << " " << sizeof(simple(df)) << std::endl;


  //auto n = unary(x * logit(x));
  //
  //for(float z = -3; z < 3; z += 0.2)
  //  std::cerr
  //    << z << " "
  //    << n->forward(z) << " "
  //    << n->backward(z) << std::endl;
  //
  //delete n;
