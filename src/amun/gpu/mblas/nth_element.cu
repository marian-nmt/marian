...
__global__ void gGetValueByKey(const float* d_in, float* d_out, int* indeces, int n)
{
...
void NthElement::getValueByKey(std::vector<float>& out, const mblas::Matrix &d_in)
{
  gGetValueByKey<<<1, lastN, 0, stream_>>>
    (d_in.data(), d_breakdown, h_res_idx, lastN);
