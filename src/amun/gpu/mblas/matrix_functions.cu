                ...
                uint beamSizeSum)
{
  ...
  std::vector<char> isFirsts = beamSizes.GetIsFirsts();
  ...
  */
  ...
  VectorWrapper<char> isFirstsWrap(d_isFirsts);
...
void AddNewData(mblas::Matrix &sourceContext,
                ...
                size_t newSentenceOffset)
{
  assert(batchId < sourceContext.dim(0));
  assert(newSentenceOffset < newSourceContext.dim(0));
