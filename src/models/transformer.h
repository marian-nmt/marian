...
public:
  ...
  Ptr<EncoderState> build(Ptr<ExpressionGraph> graph,
    ...
    auto layer = reverseTimeBatch(std::sqrt(dimEmb) * batchEmbeddings + posEmbeddings);
    auto rMask = reverseTimeBatch(batchMask);

    int heads = opt<int>("transformer-heads");
    int depth = opt<int>("enc-depth");

    for(int i = 1; i <= depth; ++i)
      layer = Layer(graph, "encoder_l" + std::to_string(i), heads, layer, rMask);
